...
const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      { href: '/tasks', label: 'Tasks', desc: 'All your tasks', icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    label: 'TRACKING',
    items: [
      { href: '/sleep', label: 'Sleep', desc: 'Track your rest', icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
        </svg>
      )},
      { href: '/energy', label: 'Energy', desc: 'Track your energy', icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
        </svg>
      )},
      { href: '/family', label: 'Family', desc: 'Family & connections', icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    label: 'INSIGHTS',
    items: [
      { href: '/assistant', label: 'AI Assistant', desc: 'AI insights & advice', icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      { href: '/work-life-balance', label: 'Work-Life', desc: 'Balance tracker', icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    label: 'ACCOUNT',
    items: [
      { href: '/settings', label: 'Settings', desc: 'App preferences', icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const renderNavItems = (onClick?: () => void) =>
    NAV_GROUPS.map((group) => (
      <div key={group.label}>
        <p className="px-3 pt-4 pb-1 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest">
          {group.label}
        </p>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-30 bg-black/40" onClick={() => setMobileOpen(false)}>
          <div className="absolute left-0 top-0 bottom-0 w-72 max-w-[85vw] bg-background shadow-xl" onClick={(e) => e.stopPropagation()}>
            <nav className="pt-16 px-3 py-4 overflow-y-auto h-full" aria-label="Mobile navigation">
