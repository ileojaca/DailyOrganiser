...
export async function POST(request: NextRequest) {
  ...
  const goals = goalsSnap.docs.map(d => ({ id: d.id, ...d.data() } as GoalData));
  ...
  const sleep = sleepSnap.docs.map(d => d.data());
  const energy = energySnap.docs.map(d => d.data());
  ...
  const avgSleep = sleep.length > 0 ? sleep.reduce((s, r) => s + (r.duration || 7), 0) / sleep.length : 7;
  const currentEnergy = energy.length > 0 ? energy[0].level : 5;

  const systemPrompt = `You are a proactive, caring personal assistant for ${profile?.fullName || 'the user'}. You know everything about their life and help them stay organized, productive, and balanced.
...
- Use tools liberally — actually DO things, don't just talk about them`;
  ...
  for (const block of response.content) {
    ...
    if (block.name === 'create_task') {
      ...
      toolResults.push({ toolName: 'create_task', result: `Task "${input.title}" created successfully`, data: { id: ref.id, title: input.title } });
    }
